...
const runCode = async (code: string, language: string, input: string): Promise<Result> => {
    ...
    const codeFilePath = path.join(execDir, fileName);
    fs.writeFileSync(codeFilePath, formattedCode);
    ...
    const command = `${compileCmd} && ${runCmd} < ${inputFilePath}`;

    const start = Date.now();

    return await new Promise((resolve, reject) => {
        exec(command, { cwd: execDir  , shell: 'true' }, (error, stdout, stderr) => {
...
const getConfig = (language: string, code: string, input: string) => {
    switch (language) {
        case 'python':
            return {
                fileName: 'script.py',
                compileCmd: `python3 -m py_compile script.py`,
