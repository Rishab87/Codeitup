    ...
    return await new Promise((resolve, reject) => {
        ...
        exec(command, { cwd: execDir}, (error, stdout, stderr) => {
            ...
            if (error) {
                ...
                resolve({ time: executionTime / 1000, memory: 0, output: stderr || error.message });
            } else {
                console.log("output");
        ...
        resolve({ time: null, memory: 0, output: (error as Error).message }
    }
    });
};
...
const getConfig = (language: string, code: string, input: string) => {
    ...
    const inputEscaped = input.replace(/\n/g, '\\n');
