  ...
  return (
    ...
    <Table>
        ...
        submissions.map((submision:any) => (
            <TableRow key={submision.id} className='h-[30px]'>
            <TableCell className={`font-semibold ${submissions.status !== "ACCEPTED" ? "text-red-500" : "text-green-500" }`}>{submision.status}</TableCell>
            ...
            <Dialog>
                <DialogTrigger asChild>
                    <Button className='h-[30px] rounded-lg mt-[3px]' variant={"outline"}>Show Code</Button>
                </DialogTrigger>
                <DialogContent>
                    <DialogHeader>
